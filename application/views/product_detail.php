...
//    $setting_site_logo = $settingData->site_logo;
    ...
    $grade_id = $productData[0]->grade_id;
    if ($grade_id == 0) {
        $grade = 0;
    }else{
       $grade = $this->db->select('grade_name')->where('grade_id',$grade_id)->from('gold_grade')->get()->row()->grade_name;
    // echo $grade; exit(); 
    }
...
?>
    	...
    	<?php
            ...
            if($prodDtaRows == 1){
                ...
                $prod_weight    = $prodDtaData[0]->weight;
                $prod_grade_id  = $prodDtaData[0]->grade_id;
                $prod_category  = $prodDtaData[0]->category;
...
<td>							<?php
   $large_file_path = '';
        ...
        if ($thumbnail == 'no_image.jpg') {
            ...
            <?php if ($prod_category == 1 OR $prod_category == 22) {
            ?>      
            <tr>
                <td style="font-family: Arial, Helvetica, sans-serif; text-align: center; font-size: 11px;">
                <?php  echo "Weight -".$prod_weight;  ?>
                </td>
            </tr>
            <?php 
                if ($prod_grade_id == 0) {
                    ...
                    $grade = $this->db->select('grade_name')->from('gold_grade')->where('grade_id',$prod_grade_id)->get()->row()->grade_name;
                }
            ?>
            <?php  ?>
