...
$paid_bys = $this->Constant_model->getddData('payment_method', 'id', 'name', 'id');
	...
	if(!empty($select_last_sales_wherehouse))
	{
	   $warehouse_tank = $select_last_sales_wherehouse->warehouse_id;
	   $product_name_code = $select_last_sales_wherehouse->product_code;
	}
	else
	{
	   $warehouse_tank = '';
	...
	<div id="PopupCustomer" class="modal fade" role="dialog">
		...
		<form id="FormStockItemSubmit" method="post" enctype="multipart/form-data">
			<div class="row">
				...
				<div class="col-md-2">
					<div class="form-group">
						<label style="margin-top: 25px;">Customer Order No</label>
						...
						<div class="col-md-3">
							...
							<div class="col-md-2">
								<div class="form-group">
									<label style="margin-top: 23px;">Sales Person </label>
													...
													<div class="row">
														<div class="col-md-2">
															<div class="form-group">
																...
																<input type="text" name="date" value="<?= date('Y-m-d H:i:s') ?>"  class="form-control" style="margin-top: 5px;" readonly="readonly" />
															</div>
														</div>
														...
														<div class="col-md-3">
															<div class="form-group">
																...
																<input type="text" readonly="readonly" name="estimate_no" class="form-control" value="<?= $getgold_orders; ?>" />
															</div>
														</div>
													...
													<div class="col-md-12" style="padding: 0px; margin-top: 10px;">
														...
														<div class="col-md-2">
															<div class="form-group">
																...
																<input type="text"  name="minPrice" id="minPrice" placeholder="0.0"  class="form-control" readonly="readonly"/>
															</div>
														</div>
														<!-- new adding end here -->
								...
								<div class="form-group">
									...
									<select name="service" id="service"  class="form-control service" style="width: 220px !important;">
										...
										<?php foreach ($getSubcategory as $subcatg_view) {
											...
											<!-- name is new by a3frt it was sub_category before -->	
											<?php } ?>
										</select>	
									</div>
								</div>
...
function calamount()
{
	...
	//$("#product_total").val(parseFloat(Product_total).toFixed(2)); //by a3frt
	$("#product_total").val(accounting.formatMoney(Product_total));

	$("#service_total").val(parseFloat(Services_total).toFixed(2));
	//$("#grand_total").val(parseFloat(gardtotal).toFixed(2)); //by a3frt
 ...
 function warehousegetdata(action)
{
	...
	$.ajax({
		type: 'POST',
		url: "<?= base_url(); ?>sales/getOutletWiseWarehouseProductNew",
   ...
   function accordingtocode(action) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			$('#product_price').val(accounting.formatMoney(data.product_price)); //new a3rft
			...
			$('#minPrice').val(accounting.formatMoney(data.ProductMinPrice)); //by a3rft
		}
	});
}
//});
...
$('#Stock_Item_submit').click(function (e) {
	...
	var gold_grade_name		= $("#grade_name").val();
		...
		var min_product_price = accounting.unformat($('#minPrice').val());
		if(min_product_price == "")
		{
			...
			else
			{
				...
				<td><input type="text" readonly name="return[' + row_item + '][product_name_code]" value="' + product_name_code + '" class="form-control" style ="width: 150px;"></td>\n\
				...
				//$('#subtotal').val(finaltotal);  //a3rft
				//$('#subtotal').val(accounting.formatMoney(finaltotal)); 
				outletdata();
...
function deletedivpay(ele) {
	...
	var totalamount = parseFloat(accounting.unformat($('#totalamount1').val()));
	var newtotal	= total_paid - removevalue;
	$("#customer_total_amount").val(newtotal);
	var formate_newtotal = accounting.formatMoney(newtotal);
	var a	= $(".tpaid").val(formate_newtotal);
	var aa	= $(".tpaid").val();
	var unformate_aa = accounting.unformat(aa);
	var b	= totalamount - unformate_aa;
	//$('.BalanceAmount').val(parseFloat(b)); //old
	$('.BalanceAmount').val(accounting.formatMoney(b)); //new
	var tpaid	 = $('.tpaid').val();
	var balant	 = $('.BalanceAmount').val();
	//$('#submitdata').hide(); // by a3frt
...
$("#FormStockItemSubmit").on('submit', (function (e) {
	...
	var tpaid = accounting.unformat($('.tpaid').val());
	var totalamount1 = accounting.unformat($('#totalamount1').val());
...
success: function (data) {
	...
	window.location.href = '<?= base_url() ?>Gold/view_sales_invoice?id=' + data;
},
});
	}
}));
...
$( document ).ready(function() 
{
	...
	$("#min_sale_req").click(function()
	{
	  ...
	  var minimum_price = accounting.unformat($("#minPrice").val());
...
//     sumofData();
// });
// $('#Stock_Item_submit').on('click',function(){
// sumofData();
// });
// function sumofData(){
