...
<html>
	<head>
		<meta charset="utf-8" />
		<title>Sale No : <?=!empty($getmainOrder->sales_id)?$getmainOrder->sales_id:''?></title>
...
<div id="wrapper">
	...
	<table class="table" cellspacing="0"  border="0"> 
		...
		<tbody> 
			...
			$grand_total=0;
			// echo "<pre>";
			// print_r($sales_invoice_item); exit();
			foreach ($sales_invoice_item as $item)
			{ 
				$total_item_qty = $total_item_qty + $item->qty;
				$subtotal = !empty($item->subtotal)?($item->subtotal):0;
				$grand_total = $grand_total + !empty($subtotal)?($subtotal):0;
				?>
			<tr>
				<td><?=$i?></td>
				...
				<td><?=!empty(($item->subtotal))?(($item->subtotal)):'0.00'?></td>
			</tr>	
			<?php
				$item_id=$item->id;
    ...
    <table class="totals" cellspacing="0" border="0" style="margin-bottom:5px; border-top: 1px solid #000; border-collapse: collapse;">
    	<tbody>
			<tr>
				...
				<td style="text-align:right;font-weight:bold;"><?php echo $grand_total; ?></td>
			</tr>
	 ...
	 <table class="totals" cellspacing="0" border="0" style="margin-bottom:5px; border-top: 1px solid #000; border-collapse: collapse;">
			...
			$paid_am=0;
			$unpaid_am = 0;
			// print_r($getPaymentSalesInvoice); exit();
			...
			<tr>
				...
				if(!empty($pay->paid_amt))
				{
					$paid_am = !empty($pay->paid_amt)?$pay->paid_amt:0;
					echo $pay->paid_amt;
				}
				else
				{
		...
		<?php	}
			?>
				...
				<tr>
					...
					<td style="text-align:right;font-weight:bold;"><?php echo !empty($paid_am )?($paid_am):0.00; ?></td>
				</tr>
				<?php
				// print_r($getmainOrder);
				...
				<tr>
					...
					<td style="text-align:right;font-weight:bold;"><?php echo !empty($paid_am )?($paid_am):0.00 + $unpaid_am; ?></td>
				</tr>
    ...
    <div id="bkpos_wrp">
    	<a href="<?=base_url()?>sales/sales_invoice" style="width:100%; display:block; font-size:12px; text-decoration: none; text-align:center; color:#FFF; background-color:#005b8a; border:0px solid #007FFF; padding: 10px 1px; margin: 5px auto 10px auto; font-weight:bold;">Back to Customer Order</a>
    </div>
    ...
    <div id="bkpos_wrp">
    	<span class="left">
    		<a href="<?=base_url()?>Gold/sales_invoice_a4?id=<?php echo $sales_id; ?>" style="width:100%; display:block; font-size:12px; text-decoration: none; text-align:center; color:#000; background-color:#4FA950; border:2px solid #4FA950; padding: 10px 0px; font-weight:bold; margin-top: 6px;">
	    		Print A4
	    	</a>
	    </span>
    </div>
