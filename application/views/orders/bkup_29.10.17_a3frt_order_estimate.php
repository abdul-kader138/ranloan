									...
									<table class="table" >
										...
										<tfoot>
											<tr>
												...
												<th ><input  readonly="" type="text" name="order_subtotal" id="order_subtotal" value="0" class="form-control" /></th>
												<th   style="text-align: right !important;">
													<div style="margin-top: 0px;">Total Order Services</div>
												</th>
												<th><input  readonly="" type="text" name="order_services_subtotal" id="order_services_subtotal" value="0" class="form-control" /></th>
												<th  style="text-align: right !important;">
													<div style="margin-top: 0px;">Grand Order Total</div>
												</th>
												<th ><input  readonly="" type="text" name="order_grand_total" id="order_grand_total" value="0" class="form-control" /></th>
												<th colspan="2"></th>
											</tr>
										</tfoot>
									</table>
...
//---------------------------------order	
        ...
        } else
        {
            ...
            var html = '<tr id="RemoveDataOrder' + row_item_order + '">\n\
				...
				<td>' + product_name + '\
					...
					<td>' + order_qty + '</td>\n\
					<td>' + order_price + '</td>\n\
					<td>' + discount_perce_oreder + '</td>\n\
					<td>' + order_tax + '</td>\n\
					<td><input type="text" readonly name="orderreturn[' + row_item_order + '][total_val_order]"		 value="' + netamount_order + '" class="form-control"></td>\n\
					<td width="12%">\n\
            ...
            var subtotalorder = $('#order_subtotal').val();
            var finaltotal = parseFloat(subtotalorder) + parseFloat(netamount_order);
            var finaltotalorder = finaltotal.toFixed(2)
            $('#order_subtotal').val(finaltotalorder);
            $('#product_name').val('');
    ...
    $(".table").delegate(".removeItemOrder", "click", function () {
        ...
        $('.totalvalOrder').val(amt_sub_totl);
    ...
    $(".order_item_remove_row").click(function () {
        ...
        var amt_sub_totl = $('.totalvalOrder').val();
        var order_total_amt = $('#order_subtotal').val();
        ...
        $('#order_service_append' + id).each(function () {
            ...
            var ord_serv_totl = $('#order_service_total').val();
            var total_order_ser = (parseFloat(ord_serv_totl) - parseFloat(totalprice)).toFixed(2);
            $("#order_service_total").val(total_order_ser);
        });
        //end remove
        ...
        //tax
        //discount order
        var ordr_discount = $('#totaldiscountOrder').val();
        var discount_grand_total = $('#discount_grand_total').val();
        var total_discount = (parseFloat(discount_grand_total) - parseFloat(ordr_discount)).toFixed(2);
        $("#discount_grand_total").val(total_discount);
        //end discount order
        var finaltotal = parseFloat(order_total_amt) - parseFloat(amt_sub_totl);
        var finaltotalorder = finaltotal.toFixed(2)
        $('#order_subtotal').val(finaltotalorder);
...
//-------------------------------------end order 
    ...
    function calamount()
    {
        ...
        var order_total = $('#order_subtotal').val();
        ...
        $('#order_grand_total').val(ordergrandtotl.toFixed(2));
        ...
        $("#order_total").val(parseFloat(order_total).toFixed(2));
