...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
   <div class="row">
      <div class="col-lg-12">
         <h1 class="page-header">Edit Product Code Numbering</h1>
      </div>
   </div>
   <div class="row">
      <div class="col-md-12">
         <div class="panel panel-default">
            <div class="panel-body">
               <div class="row">
					  ...
					  <div class="col-md-6">
					   ...
					   <div class="col-md-12 changecategorysubcategory" style="display: none;">
							<div class="form-group">
								...
								<select disabled="" class="form-control" id="category" >
									<option value="">Select Category</option>
									<?php
									foreach ($getCategory as $category)
									{
										$selected = '';
										if(!empty($getProductno->category))
										{
											if($category->id == $getProductno->category)
											{
												$selected = 'selected';
											}
										}
					   ...
					   <div class="col-md-12 changecategorysubcategory" style="display: none;">
							<div class="form-group">
								<input type="hidden" id="selected_subcategory" value="<?=!empty($getProductno->sub_id)?$getProductno->sub_id:''?>" >
								<input type="hidden" id="product_code_id" value="<?=!empty($getProductno->id)?$getProductno->id:''?>" >
								
								<label>Sub Category</label>
								<select disabled="" class="form-control" name="sub_category" id="sub_category">
				   ...
				   <div class="col-md-5" style="display: none;" id="thridPart">
					   ...
					   <div class="col-md-12" style="margin-top:40px ">
						 <div class="form-group">
							 ...
							 <input type="text" name="enter_starting_number" placeholder="1" id="enter_starting_number" value="<?=!empty($getProductno->enter_starting_number)?$getProductno->enter_starting_number:''?>" style="width: 150px;" class="form-control"  />
				...
				<div class="col-md-12" style="text-align: right;"><button class="btn btn-primary" id="SubmitBillNumbering">Update</button></div>
            </div>
         </div>
...
<script>
	<?php  
	if(!empty($getProductno->auto_generate_code))
	{ ?>
		$('#auto_number_change').click();
<?php	}
	?>
	<?php  
	if(!empty($getProductno->change_daily))
	{ ?>
		$('#dailyplay').click();
<?php	}
	else if(!empty ($getProductno->change_weekly))
	{ ?>
		$('#weeklyplay').click();
<?php	}
	else if(!empty ($getProductno->change_monthly))
	{ ?>
		$('#monthlyplay').click();
<?php	}
	else if(!empty ($getProductno->change_yearly))
	{ ?>
		$('#yearlyplay').click();
	<?php }
	
	if(!empty($getProductno->current_year))
	{ ?>
		$('#current_year').prop('checked', true);
		$('#current_year').val('1');
<?php	}
	if(!empty($getProductno->current_month))
	{ ?>
		$('#current_month').prop('checked', true);
		$('#current_month').val('1');
<?php	}
	if(!empty($getProductno->current_day))
	{ ?>
		$('#current_day').prop('checked', true);
		$('#current_day').val('1');
<?php	}
	?>
	...
	function category()
	{
		...
		$.ajax({
			type:'POST',
			url:"<?=base_url()?>setting/getSubCategory",
	...
	$('body').delegate('#SubmitBillNumbering','click',function(){
		...
		else
		{
			...
			$.ajax({
				type:'POST',
				url:"<?=base_url();?>Product_Code_Numbering/UpdateProduct_Code_Numbering",
				data: {product_code_id:product_code_id,auto_number_change: auto_number_change,
