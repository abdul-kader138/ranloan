...
?>
    ...
    if (!empty($url_name)) {
        ...
        $orderResult = $this->db->query("SELECT * FROM orders_payment WHERE vt_status = '0' $name_sort ORDER BY id DESC ");
    ...
    if (!empty($url_start) && !empty($url_end)) {
        ...
        $orderResult = $this->db->query("SELECT * FROM orders_payment WHERE vt_status = '0' $name_sort AND created_datetime >= '$start_date' AND created_datetime <= '$end_date' ORDER BY id DESC ");
        $orderData = $orderResult->result();
