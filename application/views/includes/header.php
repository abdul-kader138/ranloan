						...
						if (($rule_array['reports']->view_menu_right == 1)) {
							...
							<li class="treeview <?php if ($tk_c == 'reports') { echo 'active'; } ?>">
								...
								<ul class="treeview-menu <?php if ($tk_c != 'reports') { ?> collapse <?php } ?> <?php if ($tk_m == 'sales_report' || $tk_m == 'product_category_report') { ?> children in <?php } ?>" id="sub-item-reports">
									...
									if (array_key_exists(strtolower('product_report_weight'), $rule_array)) {
										if (($rule_array['product_report_weight']->view_menu_right == 1)) {
											?>
											<li>
												<a <?php if (($this->router->method == 'product_report_weight')) { ?> class="sub-active" <?php } ?> href="<?= base_url() ?>reports/product_report_weight">
													Product Report-Weight
												</a>
											</li>
											<?php
										}
							...
							if (($rule_array['setting']->view_menu_right == 1)) {
								...
								<li class="treeview <?php if ($tk_c == 'setting' || $tk_c == 'Bill_Numbering' || $tk_c == 'staff' || $tk_c == 'Product_Code_Numbering') { echo 'active'; } ?>">
									...
									<ul class="treeview-menu <?php if ($tk_c != 'setting') { ?> collapse <?php } ?>
										...
										if (array_key_exists(strtolower('add_loan_type'), $rule_array)) {
											if (($rule_array['add_loan_type']->view_menu_right == 1)) {
												if (ci_permissionSysAdminRole()) { ?>
													<li>
														<a <?php if (($this->router->method == 'add_loan_type') || ($this->router->method == 'add_loan_type')) {?> class="sub-active" <?php } ?> href="<?= base_url() ?>setting/add_loan_type">Add Loan Type</a>
													</li>
													<?php
												}
											}
