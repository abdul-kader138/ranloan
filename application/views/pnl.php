...
<script type="text/javascript">
	$(function () {
	    $('#pnlcontainer').highcharts({
	        ...
	        yAxis: {
	            title: {
	                text: 'Profit Amount (<?php echo $site_currency; ?>)'
	            },
	            plotLines: [{
	                value: 0,
	                width: 1,
	            	...
	            	<?php
                        for ($m = 0; $m < count($months); ++$m) {
                            ...
                            for ($d = 1; $d <= $number_of_day; ++$d) {
                                ...
                                $orderResult = $this->db->query("SELECT id, grandtotal, tax FROM orders_payment WHERE ordered_datetime >= '$full_date_start' AND ordered_datetime <= '$full_date_end' AND outlet_id = '$outlet_id' ");
