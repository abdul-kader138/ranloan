...
if(!empty($all_accept_user_id))
{
	foreach ($all_accept_user_id as  $v) 
	{
		$user_id_arr[] = $v->user_id;
	}
}

if(!empty($user_id_arr) && in_array($user_id, $user_id_arr) && $ck_pending)
{
	echo "<script> alert('Please take a action for sale in maximum/Minimum price Request !'); window.location.href = 'dashboard/make_action'; </script>";
}
			            	...
			            	<?php
                                for ($m = 0; $m < count($months); ++$m) {
                                    ...
                                    for ($d = 1; $d <= $number_of_day; ++$d) {
                                        ...
                                        $orderResult = $this->db->query("SELECT grandtotal FROM orders_payment WHERE ordered_datetime >= '$full_date_start' AND ordered_datetime <= '$full_date_end' AND outlet_id = '$outlet_id' ");
