...
if(! function_exists('ci_check_permission')) {
    ...
    function ci_check_permission($user_role){
        ...
        foreach($rules_data as $row){
            ...
            $rules_array[$resource]= $row->resource_id;
        }
        return $rules_array;
    }
}
...
if(! function_exists('ci_check_user_permission')) {
 ...
 $sql="SELECT * FROM `permissions` INNER JOIN resources on resources.id = permissions.resource_id INNER JOIN rules on rules.resource_id = permissions.resource_id WHERE permissions.user_id = '".$user_id."' and rules.role_id = '".$user_role."' ";
      	  $rules_data = $CI->db->query($sql)->result(); 
		   $rules_array=array();
		foreach($rules_data as $row){
            $rules_array[$row->name]= $row;
        }
											/*
        $name_data = $CI->Constant_model->getddData('resources','id','name', 'id');
