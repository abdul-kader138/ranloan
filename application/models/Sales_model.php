...
class Sales_model extends CI_Model
{
	...
	function select_last_sales_wherehouse($us_id)
	{
	        $this->db->select('*');
			$this->db->from('sales_invoice_item');
			$this->db->join('sales_invoice','sales_invoice.sales_id = sales_invoice_item.id','left');
			$this->db->where('sales_invoice.sales_created_user_id',$us_id);
			$this->db->limit(1);
	...
	public function old_getReserved_Item_List()
	{
		...
		$this->db->join('outlet_warehouse','outlet_warehouse.w_id = order_items_gold.warehouse_id');
		$this->db->join('inventory','inventory.product_code = order_items_gold.product_code','left');
		...
		$this->db->where('inventory.ow_id = outlet_warehouse.ow_id');
		$this->db->where('outlet_warehouse.w_id = order_items_gold.warehouse_id');
		$this->db->where('inventory.product_code = order_items_gold.product_code');
		$this->db->where('order_items_gold.work_completd_status = 1');
		...
		if(!empty($this->input->get('sub_category')))
		{
			...
			$this->db->where('sub_category.id =',$this->input->get('sub_category'));
		}
		$query = $this->db->get();
		return $query->result();
	...
	public function getSalesReserved_Item_List()
	{
		
		
	    $this->db->select('sales_invoice.*,sales_invoice_item.*,inventory.*,sub_category.sub_category as sub_category_name,sales_invoice_item.id as order_item_id');
		$this->db->from('sales_invoice');
		$this->db->join('sales_invoice_item','sales_invoice_item.sales_id = sales_invoice.sales_id','left');
		//$this->db->join('gold_order_services','gold_order_services.order_items_gold_id = sales_invoice_item.id','left');
		//$this->db->join('sub_category','sub_category.id = gold_order_services.services_name','left');
		$this->db->join('outlet_warehouse','outlet_warehouse.w_id = sales_invoice_item.warehouse_id');
		$this->db->join('inventory','inventory.product_code = sales_invoice_item.product_code','left');
		$this->db->join('products','products.code = inventory.product_code','left');
		$this->db->join('sub_category','sub_category.id = products.sub_category_id_fk','left');
		$this->db->where('inventory.outlet_id = sales_invoice.sales_outlet_id');
		$this->db->where('inventory.ow_id = outlet_warehouse.ow_id');
		$this->db->where('outlet_warehouse.w_id = sales_invoice_item.warehouse_id');
		$this->db->where('inventory.product_code = sales_invoice_item.product_code');
		$this->db->where('sales_invoice_item.work_completd_status = 0');
		$this->db->where('inventory.qty <= 0');
	...
	*/
	...
	public function getOutletWiseWarehouseProduct($outlet_id,$warehouse_id,$type)
	{
		...
		public function getOutletWiseWarehouseProductNew($outlet_id,$warehouse_id,$type)
	{
		$this->db->select('inventory.*,products.name as prdocuname,products.id as product_id ');
		$this->db->from('inventory');
		$this->db->join('products','products.code = inventory.product_code');
		...
		$this->db->where('outlet_warehouse.w_id',$warehouse_id);
		$this->db->where('inventory.type',$type);
	...
	public function getDataInventoryWise($pcode,$outlet_id,$type,$WareHouse)
	{
		...
		$this->db->where('outlet_id',$outlet_id);
		$this->db->where('type',$type);
		$this->db->where('outlet_warehouse.w_id',$WareHouse);
		$this->db->where('inventory.ref_number != ""');
		$this->db->limit(1);
		$query = $this->db->get();
		return $query;	
	}
