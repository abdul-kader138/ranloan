...
Class Store extends CI_Controller{
	...
	public function SubmitTransferStore()
	{
		$ins_data = array(
			...
			// 'receive_store_id'	    => $this->input->post('store_id'),
			'grade_id'				=> $this->input->post('grade_id'),
			...
			'status'				=> '1',
			'product_add_from'		=> 'Transfer bulk',
		...
		$product_report = array(
			...
			'sales_qty'		=> 0,
			'balance_qty'   => 1,
			'opening_wt'	=> $this->input->post('TotalGoldweight'),
			// 'product_wt'	=> $this->input->post('code'),
			'purchase_wt'	=> 0,
			'sales_wt'		=> 0,
			'balance_wt'   => $this->input->post('TotalGoldweight'),
			// 'product_type'  =>'bulk',
		...
		if (count($ckPcodeData) == 0) {
			...
			$product_balance_wt = $opening_balance_wt - $this->input->post('TotalGoldweight');
 ...
 // print_r($sheet_data);
 			 		foreach ($sheet_data as $data)
 			 		{
 			 			if($i != 0)
 			 			{
 			 				...
 			 				$productWeight  = !empty($data['H']) ? $data['H'] : 0;
 			 				$stoneWeight  = !empty($data['I']) ? $data['I'] : 0;
 			 				$wastage = !empty($data['J']) ? $data['J'] : 0;
 			 				$stoneCost = !empty($data['K']) ? $data['K'] : 0;
 			 				$labourCost = !empty($data['L']) ? $data['L'] : 0;
 			 				$otherCost1 = !empty($data['M']) ? $data['M'] : 0;
 			 				$otherCost2 = !empty($data['N']) ? $data['N'] : 0;
 			 				$otherCost3 = !empty($data['O']) ? $data['O'] : 0;
