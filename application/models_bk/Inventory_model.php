...
class Inventory_model extends CI_Model
{
    ...
    {
		$this->db->select('products.start_qty as product_qty,products.product_type,products.outlet_id_fk,products.grade_id,products.NetGoldWeight, products.Wastagegold, products.TransferredCost, products.TotalAllOtherCost, category.name as category_name, sub_category.sub_category, products.category, sum(inventory.qty) as qty, (SELECT grade_name FROM `gold_grade` WHERE `grade_id` = products.grade_id) as gold_grade, inventory.gold_weight,inventory.type as inventorytype, group_concat(inventory.ow_id) as ow_id, products.code, products.name, products.purchase_price, products.weight, outlets.name as outletname, outlets.name as outletname');
